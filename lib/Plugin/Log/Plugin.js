...
_.onTwitch = function(message) {
	var user = message.getUsername(), chan = this.getChan(message), cmd = message.getCommand();

	if (chan in this.channels && (cmd === 'clearchat' || cmd === 'subscriber')) {
		this.channels[chan].twitchMessage(chan, user, message);
	}
};
