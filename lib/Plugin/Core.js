...
CorePlugin.prototype.shutdown = function() {
	...
	var self = this, twitchClient = bot.getTwitchClient();

	setTimeout(function() {
		self.debug('Going to quit now.');

		twitchClient.irc.disconnect('Bye and thanks for all the fish.', function() {
			if (twitchClient.senderInterval) {
				clearInterval(twitchClient.senderInterval);
			}
