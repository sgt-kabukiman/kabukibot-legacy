...
DomainBanPlugin.prototype = {
	setup: function(kabukibot, eventDispatcher) {
		...
		if (this.bot.getConfig().database.driver === 'sqlite') {
			...
			this.db.query('CREATE TABLE IF NOT EXISTS ' + this.table + ' (channel VARCHAR(200), domain VARCHAR(200), bantype VARCHAR(20), PRIMARY KEY (channel, domain)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB');
		}
	},
	...
	unbanDomain: function(chan, args, message) {
		...
		message.respond('links to ' + domain + ' will no longer be ' + (ban.bantype === 'ban' ? 'banned' : ('timed out for ' + utils.secondsToTime(ban.timeout))) + '.');
