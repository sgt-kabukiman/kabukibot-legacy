...
DomainBanPlugin.prototype = {
	...
	banDomain: function(chan, args, message) {
		...
		if (args.length < 1) {
			message.respond('Syntax is `!ban_domain example.com [timeout N]`. If no timeout is given, the sender will be *banned*.');
			return;
		}
		...
		if (domain === null) {
			message.respond('Sorry, I could not understand that domain name. Use something like "example.com" (without the quotes).')
			return;
		}
	...
	unbanDomain: function(chan, args, message) {
		var domain, exists, ban;

		if (args.length < 1) {
			message.respond('Syntax is `!unban_domain example.com`.');
			return;
		}
