...
ACLPlugin.prototype = {
	...
	onCommand: function(command, args, message) {
		...
		// not for us
		if (command !== this.prefix+'allow' && command !== this.prefix+'deny' && command !== this.prefix+'permissions') {
			return;
		}
		...
		if (command === this.prefix+'permissions') {
			...
			message.respond('Available permissions are: ' + (tokens.length === 0 ? '(none)' : tokens.join(', ')));
			return;
		}

		// no token or no user ident given
		if (args.length < 2) {
			message.respond('Syntax is `!' + this.prefix + 'allow permission ident[,ident...]` -- ident can be ' + ACL.GROUPS.join(', ') + ' or any username; !' + this.prefix + 'deny works the same way. See !' + this.prefix + 'tokens for a list of available tokens.');
			return;
		}
		...
		if (token.length === 0 || tokens.indexOf(token) === -1) {
			message.respond('Invalid permission (' + args[0] + ') given.');
			return;
		}
	...
	getTokens: function(channel) {
		var
			...
			result  = [],
			pluginKey, pluginTokens, idx;

		for (pluginKey in plugins) {
			pluginTokens = plugins[pluginKey].getACLTokens();

			for (idx in pluginTokens) {
				result.push(pluginTokens[idx]);
			}
		}
