...
ACLPlugin.prototype.onCommand = function(command, args, message) {
	...
	if (command === this.gcmd('permissions')) {
		...
		return message.respond('available permissions are: ' + (permissions.length === 0 ? '(none)' : utils.humanJoin(permissions)));
	}
	...
	if (command === this.gcmd('allowed')) {
		...
		return message.respond('"' + permission + '" is granted to: ' + (permissions.length === 0 ? '(nobody)' : utils.humanJoin(permissions)));
	}

	// no user ident(s) given
	if (args.length < 2) {
		return this.errorResponse(message, 'no groups/usernames given. Group names are ' + utils.humanJoin(ACL.GROUPS) + '.');
	}
...
ACLPlugin.prototype.handleAllowDeny = function(channel, command, permission, args, message, silent) {
	...
	if (processed.length === 0) {
		...
		if (!silent) message.respond('granted permission for ' + permission + ' to ' + utils.humanJoin(processed) + '.');
	}
	else {
		if (!silent) message.respond('revoked permission for ' + permission + ' from ' + utils.humanJoin(processed) + '.');
	}
