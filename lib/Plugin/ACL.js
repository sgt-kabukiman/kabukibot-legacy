...
ACLPlugin.prototype = {
	...
	handleAllowDeny: function(channel, command, permission, args, message, silent) {
		...
		if (args.length === 0) {
			if (!silent) message.respond(this.bot.getErrorResponse() + 'invalid groups/usernames. Use a comma separated list if you give multiple.');
			return false;
		}
		...
		if (processed.length === 0) {
			if (!silent) message.respond(this.bot.getErrorResponse() + 'no changes needed.');
		}
		else if (isAllow) {
