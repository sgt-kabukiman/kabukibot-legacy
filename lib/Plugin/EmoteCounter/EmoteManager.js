...
EmoteManager.prototype = {
	...
	fetchEmotes: function() {
		...
		http.get('https://api.twitch.tv/kraken/chat/emoticons', function(data) {
			...
			self.fetchFrankerFaceZEmotes(1, function() {
				log.debug('EmoteManager: Success.');
	...
	parseTwitchEmotes: function(data) {
		...
		for (i = 0; i < total; ++i) {
			...
			// some regexes are simply skipped because they are retarded
	...
	fetchFrankerFaceZEmotes: function(page, callback) {
		var self = this;
		...
		this.log.debug('EmoteManager: Fetching FFZ emotes page ' + page + '...');
		http.get('http://api.frankerfacez.com/v1/emoticons?per_page=200&page=' + page, function(data, err) {
			if (err || !data) {
				callback();
				return;
			}

			self.parseFrankerFaceZEmotes(JSON.parse(data));

			setTimeout(function() {
				self.fetchFrankerFaceZEmotes(page + 1, callback);
			}, 2000);
		});
	},
	...
	parseFrankerFaceZEmotes: function(data) {
		for (var i = 0; i < data.emoticons.length; ++i) {
			var emote   = data.emoticons[i];
			var channel = emote.owner.name;

			if (!(channel in this.emotes.frankerfacez)) {
				this.emotes.frankerfacez[channel] = [];
			}

			this.emotes.frankerfacez[channel].push(emote.name);
		}
	},
