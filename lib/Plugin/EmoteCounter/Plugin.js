...
EmoteCounterPlugin.prototype = {
	setup: function(kabukibot, eventDispatcher) {
		...
		if (this.bot.getConfig().database.driver === 'sqlite') {
			...
			this.db.query('CREATE TABLE IF NOT EXISTS ' + this.table + ' (channel VARCHAR(200), emote VARCHAR(50), counter INTEGER UNSIGNED, PRIMARY KEY (channel, emote)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB');
		}
	},
	...
	emoteCount: function(chan, args, message) {
		...
		if (count === 0) {
			return message.respond(emote + ' has not yet been used or does not even exist.');
		}
		else if (count === 1) {
			return message.respond(emote + ' has been used once.');
		}
		else {
			return message.respond(emote + ' has been used ' + numeral(count).format('0,0') + ' times.');
		}
	},
