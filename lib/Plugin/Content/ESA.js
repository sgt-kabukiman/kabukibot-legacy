...
ESAContentPlugin.prototype = {
	...
	onCommand: function(command, args, message) {
		...
		switch (command) {
			case 'esa':
				message.respond('ESA stands for European Speedster Assembly, an annual charity speedrunning marathon (like AGDQ) held in Sweden. It\'s kind of the "European AGDQ".');
				return;

			case 'esa2014':
				message.respond('ESA 2014 is at Nyeport in Skövde (Sweden), from July 27th to August 3rd (main marathon); stream is open from the 25th (setup) to the 6th (clean up and bonus stream).');
				return;
		}
	}
};
