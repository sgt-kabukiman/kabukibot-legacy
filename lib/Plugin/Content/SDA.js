...
_.setup = function(kabukibot, eventDispatcher) {
	...
	if (kabukibot.getConfig().plugins.sda.tracker) {
		...
		// update every minute
		this.interval = setInterval(this.updateTracker.bind(this), 60*1000);
		this.updateTracker();
	}
};
...
_.onCommand = function(command, args, message) {
	...
	if (command.match(/s?gdq(stats|donations)/i)) {
		...
		message.respondToAll('We are at ' + donations + ' donations, with a total of $' + total + ' raised for the Prevent Cancer Foundation ' + emotes.join(' '));
	}
	else {
