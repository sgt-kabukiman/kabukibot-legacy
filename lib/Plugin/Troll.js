...
_.onCommand = function(command, args, message) {
	...
	// not for us
	if (!(command in this.responses) && command !== 'uptime') {
		return;
	}
	...
	if (command === 'uptime') {
		message.respondToAll('This channel has been live for ' + utils.secondsToTime(getRandomUptime(), false) + '.');
	}
	else {
