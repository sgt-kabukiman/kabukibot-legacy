...
function normalize(s) {
	return s.replace(/[^a-z ]/i, '').toUpperCase();
}
...
HangManPlugin.prototype.guess = function(chan, args, message) {
	...
	if (state.term.indexOf(letter) === -1) {
		message.respond(rand(this.missMessages).replace('%letter%', letter));
	...
	if (this.isSolved(state)) {
		message.respond(rand(this.winMessages).replace('%letter%', letter));
	...
	// this was a successful guess, but the game keeps on
		...
		message.respond(rand(this.hitMessages).replace('%letter%', letter));
		message.respondToAll(this.hideLetters(state.term, state.letters), true);
...
HangManPlugin.prototype.solve = function(chan, args, message) {
	...
	if (solution !== secret) {
		return message.respond(rand(this.missedSolveMessages).replace('%solution%', solution));
	}
	...
	message.respondToAll(rand(this.solvedMessages).replace('%user%', message.getUser().getName()));
	message.respondToAll('The solution is: ' + state.term, true);
	message.respondToAll('This game was brought to you by ' + state.author + '.', true);
