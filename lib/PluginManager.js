...
function PluginManager(database, log) {
	this.db         = database;
	this.log        = log;
	this.plugins    = [];
	this.map        = {};
...
PluginManager.prototype = {
	...
	setupChannel: function(channel) {
		...
		this.db.select('plugin', '*', { channel: name }, function(err, rows) {
			var i, len;
	...
	teardownChannel: function(channel) {
		...
		delete this.loaded[chan];

		this.log.debug('Plugins for #' + chan + ' have been torn down.');
	},
