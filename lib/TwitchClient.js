...
var TwitchClient = function(ircClient, config) {
	this.irc          = ircClient;
	this.config       = config;
	this.channels     = {};
	this.plugins      = [];
...
TwitchClient.prototype = {
	...
	joinChannel: function(channel) {
		...
		this.irc.join(channel.getIrcName(), function(chan, message) {
			console.log('INFO: joined ' + message.args[0]);
		});
	...
	addHomelessSubscriber: function(username) {
		console.log('Added ' + username + ' as a homeless subscriber.');
